...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSpritInit);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnHealthFull);
...
public:
	...
	void Player_Scale_Init();

	UPROPERTY(BlueprintAssignable)
	FOnHealthFull OnHealthFull;
private:
	...
	//타겟된 적 리스트
	TSet<ABeatUnitCharacter_Enemy*> hit_list;
	//오버랩된 적 리스트
	TSet<ABeatUnitCharacter_Enemy*> enemy_area;
	...
	UPROPERTY()
	FVector Init_PlayerScale;
	UPROPERTY()
	float Init_PlayerAttackRange;
	UPROPERTY()
	float Init_PlayerDashRange;

	void UpdateMouseCursorRotation();

	void HandleObstacleInFront();
