...
void ALinearProjectile::ConstructComponent()
{
	...
	//투사체 꼬리 나이아가라 시스템 컴포넌트 추가
	ProjectileTailFxComponent = CreateDefaultSubobject<UNiagaraComponent>(TEXT("Projectile Tail Fx"));
	ProjectileTailFxComponent->SetupAttachment(HitCollisionComponent);
	static ConstructorHelpers::FObjectFinder<UNiagaraSystem> ProjectileTailFxAsset(TEXT("/Game/FX/NS_ProjectileTail.NS_ProjectileTail"));
	if (ProjectileTailFxAsset.Succeeded())
	{
		ProjectileTailFxComponent->SetAsset(ProjectileTailFxAsset.Object);
	}
	else
	{
		LOG_DEBUG(Error, TEXT("ProjectileTailFxAsset Can't Load"));
	}
...
void ALinearProjectile::Init()
{
	...
	ProjectileTailFxComponent->SetVariableFloat(TEXT("FxLength"), ProjectileTailFxLength);
}
