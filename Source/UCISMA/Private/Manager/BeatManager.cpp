...
void ABeatManager::SetupStage()
{
	...
	LastBeatIndex = BeatPerSecond * StageManager->GetSongLength() + StageManager->GetBeatUpdateCallRate();
...
void ABeatManager::StartBeat()
{
	...
	LOG_PSJ(Display, TEXT("BeatUpdate Call Rage : %f"), SecondPerBeat / StageManager->GetBeatUpdateCallRate())
	// 1/4박자마다 BeatUpdate를 실행시킨다.
	GetWorld()->GetTimerManager().SetTimer(BeatTimerHandle, this, &ABeatManager::BeatUpdate, SecondPerBeat / StageManager->GetBeatUpdateCallRate(), true, StageManager->GetDelayForSongAndBeat() + InputOffset);
}
...
void ABeatManager::OnEveryBeat()
{
	...
	*/
	StageManager->AddScore(EScoreType::EveryBeat);
}

void ABeatManager::OnEveryQuaterBeat()
{
	...
	if (QuaterBeatIndex == StageManager->GetActionDelayQuaterBeat()[EJudgementAction::Attack])
	{
		PlayMetronomeSound();
	}
	else if (QuaterBeatIndex == StageManager->GetActionDelayQuaterBeat()[EJudgementAction::Dash])
	{
		PlayMetronomeSound();
	}
}
...
void ABeatManager::UpdateActionsActivation()
{
	...
	{
		if (QuaterBeatIndex == (StageManager->GetBeatUpdateCallRate()/ 2 + DelayQuaterBeat.Value) % StageManager->GetBeatUpdateCallRate())
		{
...
{
	//QuaterBeatIndex == BeatUpdateCallRate 즉, 한 박자가 모두 지날 경우 비트 인덱스 갱신
	QuaterBeatIndex++;
	if (QuaterBeatIndex == StageManager->GetBeatUpdateCallRate())
	{
		QuaterBeatIndex = 0;
		BeatIndex++;
	}
...
EJudgementResult ABeatManager::CheckJudgement(EJudgementAction Action)
{
	...
	float ElapsedTimeFromBeat = GetWorldTimerManager().GetTimerElapsed(BeatTimerHandle) + (SecondPerBeat / static_cast<float>(StageManager->GetBeatUpdateCallRate())) * (QuaterBeatIndex - StageManager->GetActionDelayQuaterBeat()[Action]);
	float RemainingTimeFromBeat = GetWorldTimerManager().GetTimerRemaining(BeatTimerHandle) + (SecondPerBeat / static_cast<float>(StageManager->GetBeatUpdateCallRate())) * (StageManager->GetBeatUpdateCallRate() - 1 - (QuaterBeatIndex - StageManager->GetActionDelayQuaterBeat()[Action]));
