...
bool AUnitManager::CompareBeatUnit(AActor& A, AActor& B)
{
	...
	return StageManager->GetBeatUpdateCallRate() * BeatUnitA->GetBeatIndex() + BeatUnitA->GetQuaterBeatIndex() < StageManager->GetBeatUpdateCallRate() * BeatUnitB->GetBeatIndex() + BeatUnitB->GetQuaterBeatIndex();
}
...
void AUnitManager::Init(AStageManager* InputStageManager)
{
	...
	BeatUnits.Sort([this](AActor& A, AActor& B) { return CompareBeatUnit(A, B); });
