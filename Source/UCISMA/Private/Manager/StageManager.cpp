...
void AStageManager::InitVariables()
{
	...
	bIsInitStageComplete = false;
}
#if WITH_EDITOR
bool AStageManager::GetCameraLocationAtBeatIndex(int BeatIndex, int QuaterIndex, FVector& OutLocation)
{
	// 유효성 검사
	if (!MapArray)
	{
	...
	//비트
	float BeatTime = BeatIndex * SecondPerBeat + ((float)QuaterIndex / BeatUpdateCallRate) * SecondPerBeat;
	...
	while (CurrentIndex < Positions.Num())
	{
		...
		float MovePerBeat = (BeatPerSecond / static_cast<float>(BeatUpdateCallRate)) * Camera_Speed * TileSpeed;
		float BeatsToTraverseTile = 0.f;

		// MovePerBeat가 0일 경우 
		// Camera_Speed = 0 || TileSpeed = 0 밖에 없음
		if (MovePerBeat > 0.f)
		{
		...
		{
			if (Camera_Speed == 0.f)  LOG_KHJ_SCREEN(3.f, TEXT("현재 카메라 속도가 0입니다.\n 스테이지 데이터를 확인하시거나 StageManager에서 데이터를 로드하세요"));
			BeatsToTraverseTile = 1 / (BeatPerSecond / static_cast<float>(BeatUpdateCallRate)); // 또는 원하는 정지 길이 비트 수
		}
...
bool AStageManager::SaveStageRecord(bool bIsComplete)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("BaseScore=%.2f"), BaseScore);
	UE_LOG(LogTemp, Warning, TEXT("GetThreshold_S=%.2f"), BaseScore * GameDataAsset->GetThreshold_S());
	UE_LOG(LogTemp, Warning, TEXT("GetThreshold_A=%.2f"), BaseScore * GameDataAsset->GetThreshold_A());
	UE_LOG(LogTemp, Warning, TEXT("GetThreshold_B=%.2f"), BaseScore * GameDataAsset->GetThreshold_B());
	...
	bool bOverScore_S = Score >= BaseScore * GameDataAsset->GetThreshold_S() * 0.01;
	bool bOverScore_A = Score >= BaseScore * GameDataAsset->GetThreshold_A() * 0.01;
	bool bOverScore_B = Score >= BaseScore * GameDataAsset->GetThreshold_B() * 0.01;
...
void AStageManager::AddScore(EScoreType ScoreType)
{
	if (ScoreType == EScoreType::KillEnemy)
	{
		AddScore(100);
		return;
	}
	if (ScoreType == EScoreType::EveryBeat)
	{
		AddScore(10);
		return;
	}
	if (ScoreType == EScoreType::Recover)
	{
		AddScore(500);
		return;
	}
...
int AStageManager::GetLastBeatIndex() 
{ 
	if (BeatManager == nullptr)
	{
		InitStage();
	}
	return BeatManager->GetLastBeatIndex(); 
}
...
float AStageManager::GetBaseScore(EStageIndex stageindex) {
	...
	return GameDataAsset->GetStageMaxScore(stageindex);
...
int AStageManager::GetBeatUpdateCallRate()
{
	if (BeatUpdateCallRate <= 0)
	{
		BeatUpdateCallRate = 4;
	}

	return BeatUpdateCallRate;
}
