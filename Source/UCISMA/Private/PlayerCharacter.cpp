...
void APlayerCharacter::Set_Material()
{
	if (UMeshComponent* MeshComponent = GetMesh())
	{
		struct FSlotInfo
		{
			...
			int32 DuplicateSlot = -1; // 같은 재질을 다른 슬롯에도 적용할 경우
		};

		FSlotInfo SlotInfos[] = {
			{0, &DynamicMaterial0},
			{1, &DynamicMaterial1_2, 2},
			{3, &DynamicMaterial3}
		};

		for (const FSlotInfo& Info : SlotInfos)
		{
			if (UMaterialInterface* Material = MeshComponent->GetMaterial(Info.SlotIndex))
			{
				UMaterialInstanceDynamic* DynMat = UMaterialInstanceDynamic::Create(Material, this);
				if (DynMat)
				{
					MeshComponent->SetMaterial(Info.SlotIndex, DynMat);
					if (Info.DuplicateSlot >= 0)
						MeshComponent->SetMaterial(Info.DuplicateSlot, DynMat);

					if (Info.OutDynamicMaterial)
						*Info.OutDynamicMaterial = DynMat;
				}
			}
		}
	}
...
void APlayerCharacter::Set_PlayerSpeed()
{
	...
	if (CamActor->GetDirection() == PlayerDirection)
	{
		SpeedMultiplier = (PlayerDirection == ECamDirection::Right || PlayerDirection == ECamDirection::Left)
			? Player_Horizontal_Speed
			: Player_Vertical_Speed;
	}

	// 최종 속도 적용
	GetCharacterMovement()->MaxWalkSpeed = Player_Default_Speed * SpeedMultiplier * bpm_speed;
}
...
void APlayerCharacter::System_Target()
{
	...
	for (ABeatUnitCharacter_Enemy* Enemy : enemy_area)
	{
		...
		const float DistanceSq = FVector::DistSquared(PlayerLocation, EnemyLocation);
	...
	const FVector EnemyLocation = TargetEnemy->GetMesh()->GetComponentLocation();
	const FVector Direction = (EnemyLocation - PlayerLocation).GetSafeNormal();
	const FRotator TargetRotation = FRotationMatrix::MakeFromX(Direction).Rotator();
	SetActorRotation(FRotator(0.f, TargetRotation.Yaw, 0.f)); // Pitch 제거

	const float Distance = FMath::Sqrt(MinDistanceSq);
	const FVector ArrowLocation = FVector(PlayerLocation.X, PlayerLocation.Y, 0.01f);
	const float SizeFactor = Distance * 0.01f;

	float UV = Arrow_Length;
	if (Distance >= 100.0f)
	{
		const int32 Steps = FMath::FloorToInt((Distance - 100.0f) / 50.0f) + 1;
	...
	if (!ArrowActor)
	{
		...
		if (UStaticMeshComponent* MeshComp = ArrowActor->FindComponentByClass<UStaticMeshComponent>())
		{
	...
	if (arrowMat)
	{
		arrowMat->SetScalarParameterValue("UV", UV);
	}
...
void APlayerCharacter::Tick(float DeltaTime)
{
	...
	UpdateMouseCursorRotation();

	HandleDash(DeltaTime);
	ShowWidgetOnBlockedView();
	HandleObstacleInFront();
}
...
void APlayerCharacter::Mobile_Move(const FInputActionValue& Value) {
	...
	if (StageManager->IsTutorialStage())
	{
		// 입력을 감지할 최소한의 값 (데드존 역할)
		const float AxisThreshold = 0.5f;
		const float AbsX = FMath::Abs(MovementVector.X);
		const float AbsY = FMath::Abs(MovementVector.Y);

		// 대각선 입력을 방지하고 상하/좌우 중 더 크게 입력된 축을 우선 처리
		if (AbsY > AbsX)
		{
			// 상하(Y축) 입력이 더 클 때
			if (MovementVector.Y > AxisThreshold)
			{
				OnTutorial_W(); // 위로 이동
			}
			else if (MovementVector.Y < -AxisThreshold)
			{
				OnTutorial_S(); // 아래로 이동
			}
		}
		...
		{
			// 좌우(X축) 입력이 더 클 때
			if (MovementVector.X < -AxisThreshold)
			{
				OnTutorial_A(); // 왼쪽으로 이동
			}
			else if (MovementVector.X > AxisThreshold)
			{
				OnTutorial_D(); // 오른쪽으로 이동
			}
		}
...
void APlayerCharacter::Player_State_Init() {
	...
	Init_PlayerScale = RootComponent->GetComponentScale();
	Init_PlayerAttackRange = StageManager->GetPlayerRange();
	Init_PlayerDashRange = StageManager->GetPlayerDashRange();
...
void APlayerCharacter::Player_Scale_Init() {
	RootComponent->SetWorldScale3D(Init_PlayerScale);
	SetPlayerScale(Init_PlayerAttackRange, Init_PlayerDashRange);
}
...
void APlayerCharacter::Take_HP(int count, AActor* DamageCauser)
{
	...
	if (count > 0) {
		...
		StageManager->AddScore(EScoreType::Recover);
...
void APlayerCharacter::OnSwordMeshOverlap(
...
{
	// 한 번만 캐스팅
	if (ABeatUnitCharacter_Enemy* Enemy = Cast<ABeatUnitCharacter_Enemy>(OtherActor))
	{
		// 적이 유효하고 공격 대상이면
		if (Check_Hit_Enemy(Enemy))
		{
...
bool APlayerCharacter::Check_Hit_Enemy(ABeatUnitCharacter_Enemy* Enemy) {
	return Enemy && enemy_area.Contains(Enemy);
}
...
void APlayerCharacter::InitSprit() {
	OnSpritInit.Broadcast();
}

void APlayerCharacter::UpdateMouseCursorRotation() {
	if (IsPC && CurrentInputMode == EInputMode::WASDMovement && canrotation) {
		if (PlayerController)
		{
			FHitResult HitResult;
			...
			if (PlayerController->GetHitResultUnderCursor(ECC_Visibility, false, HitResult))
			{
				const FVector CursorLocation = HitResult.Location;

				const FVector CharacterLocation = GetActorLocation();

				const FVector Direction = CursorLocation - CharacterLocation;
...
void APlayerCharacter::HandleObstacleInFront()
{
	if (!overlap_wall || !overlap_obstacle || !IsObstacleInFront())
		return;

	UStaticMeshComponent* MeshComp = Cast<UStaticMeshComponent>(OverlappingComponent);
	if (!MeshComp && OverlappingComponent)
	{
	...
	static UMaterialInterface* ObstacleMat = nullptr;
	if (!ObstacleMat)
		ObstacleMat = LoadObject<UMaterialInterface>(nullptr,
			TEXT("Material'/Game/Assets/SpawnPlane/spawn_title_material1.spawn_title_material1'"));

	if (ObstacleMat)
		Obstacle_SetMaterial(MeshComp, ObstacleMat);

	MeshComp->SetCollisionEnabled(ECollisionEnabled::NoCollision);
}

void APlayerCharacter::HandleDash(float DeltaTime)
{
	...
	FVector NewLocation = FMath::Lerp(StartLocation, EndLocation, FMath::Clamp(Alpha, 0.f, 1.f));
	...
	if (Hit.IsValidBlockingHit() && Hit.Component->GetCollisionObjectType() != ECC_GameTraceChannel3)
	{
		EndDash();
	}
...
void APlayerCharacter::EndDash()
{
	...
	if (Dash_NiagaraComp) Dash_NiagaraComp->SetVisibility(false);
	if (PerfectDashComponent) PerfectDashComponent->Deactivate();
