...
int ACamera_Map::BeatIndexToTileIndex(int BeatIndex, float* OutLerpAlpha)
{
	...
	int BeatOfTile = StageManager->GetBeatUpdateCallRate() / Tile_Speed_Index[0];
	...
	while (RemainingBeat >= BeatOfTile)
	{
		...
		ReturnTileIndex++;
		BeatOfTile = StageManager->GetBeatUpdateCallRate() / Tile_Speed_Index[ReturnTileIndex];
	}
	if (OutLerpAlpha)
	{
...
void ACamera_Map::InitTileStartBeatIndex()
{
	...
	for (int NowBeat = 0; NowBeat <= LastBeatIndex; NowBeat++)
	{
		...
		if (Beat_Skip_Index.Contains(NowBeat))
		{
			...
			NextTileBeat = NextTileBeat + StageManager->GetBeatUpdateCallRate() * 1 / Tile_Speed_Index[Beat_Skip_Index[NowBeat]+1];
		...
		if (NowBeat == NextTileBeat)
		{
			...
			{
				NextTileBeat = NextTileBeat + StageManager->GetBeatUpdateCallRate() * 1 / Tile_Speed_Index[NextTileIndex];
			}
			else
			{
				NextTileBeat = -1;
