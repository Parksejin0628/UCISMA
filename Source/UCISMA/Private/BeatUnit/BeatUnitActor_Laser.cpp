...
void ABeatUnitActor_Laser::BeatUpdate(int QuaterIndex)
{
    ...
    case ELaserState::Preparing:
        ...
        //나중에 바꿀수 있게 ㅇㅇ뭔말알?
        if (StateTimer == 0 * StageManager->GetBeatUpdateCallRate())        NewColor = LaserColorBeat1;
        else if (StateTimer == 1 * StageManager->GetBeatUpdateCallRate())   NewColor = LaserColorBeat2;
        else if (StateTimer >= 2 * StageManager->GetBeatUpdateCallRate())   NewColor = LaserColorBeat3;
    ...
    case ELaserState::Disappearing:
        if (StateTimer > 0.5 * StageManager->GetBeatUpdateCallRate()) DeactiveUnit();
        break;
    }
}
...
void ABeatUnitActor_Laser::CheckStateTime()
{
    ...
    {
        if (StateTimer >= 3 * StageManager->GetBeatUpdateCallRate())
        {
...
{
    //Destroy(); // 레이저 제거
    if (StateTimer <= 0.5 * StageManager->GetBeatUpdateCallRate())
    {
        TimeLine += delta;
