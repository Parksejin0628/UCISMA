...
void ABeatUnitActor_VFXBox::ChangeOpacity()
{
	...
	const int TotalQuterBeatIndex = (EndQuterBeatIndex + EndBeatIndex * stageManager->GetBeatUpdateCallRate()) - (QuaterBeatIndex + BeatIndex * stageManager->GetBeatUpdateCallRate());
	OpacityDuration = stageManager->GetSecondPerBeat() * TotalQuterBeatIndex / stageManager->GetBeatUpdateCallRate();
	ElapsedTime = 0.f;
